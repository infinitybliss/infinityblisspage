import type { Dictionary } from "@/types/dictionary";

export const gl: Dictionary = {
  meta: {
    title: "Infinity Bliss | Nails & Massage en Santiago de Compostela",
    description:
      "Masaxes e tratamentos de benestar en Santiago de Compostela para peregrinos e clientes locais.",
  },
  brand: {
    name: "Infinity Bliss",
    tagline: "Nails & Massage",
  },
  skipToContent: "Saltar ao contido",
  nav: {
    home: "Inicio",
    services: "Servizos",
    pilgrims: "Peregrinos",
    about: "Sobre nós",
    contact: "Contacto",
    book: "Reservar",
    language: "Idioma",
    openMenu: "Abrir menú",
    closeMenu: "Pechar menú",
    main: "Navegación principal",
  },
  hero: {
    title: "O teu momento de benestar en Santiago",
    text: "Masaxes e tratamentos de benestar pensados para recuperar o corpo, desconectar e volver a sentirte ben.",
    primaryCta: "Reservar cita",
    secondaryCta: "Ver servizos",
    imageLabel: "Fotografía",
    imageAlt: "Espazo reservado para unha fotografía do centro ou dun tratamento",
  },
  intro: {
    eyebrow: "Santiago de Compostela",
    title: "Un espazo para deterte e coidar o corpo",
    text: "Infinity Bliss nace como un centro de masaxes e benestar en Santiago, pensado tanto para quen chega despois do Camiño como para quen busca un coidado habitual preto de casa.",
  },
  services: {
    eyebrow: "Coidados",
    title: "Servizos destacados",
    text: "Unha selección dos nosos tratamentos de masaxe, rituais e spa en Santiago de Compostela.",
    pageEyebrow: "Tratamentos",
    pageTitle: "Servizos de benestar",
    pageDescription:
      "Descubre o noso catálogo de masaxes, rituais e experiencias de spa. Consulta duracións, prezos e reserva a túa cita.",
    pilgrimsIntro:
      "Tratamentos especialmente pensados para recuperar pernas, pés, costas e corpo despois das etapas do Camiño de Santiago.",
    viewTreatment: "Ver tratamento",
    fromPrice: "desde",
    bookCta: "Reservar cita",
    bookThisTreatment: "Reservar este tratamento",
    bookDuration: "Reservar",
    benefitsTitle: "Beneficios",
    durationsTitle: "Duracións e prezos",
    pricingInfoTitle: "Prezo",
    askAvailability: "Consultar dispoñibilidade",
    availabilityPending:
      "A reserva en liña deste tratamento estará dispoñible proximamente. Contacta connosco para consultar dispoñibilidade.",
  },
  upcoming: {
    eyebrow: "Novidades",
    title: "Proximamente en Infinity Bliss",
    text: "Seguimos preparando novas formas de coidarte. Estes servizos estarán dispoñibles proximamente.",
    homeTitle: "Proximamente",
    homeText:
      "Seguimos preparando novas formas de coidarte. Estes servizos estarán dispoñibles proximamente.",
    badge: "Proximamente",
    availableSoon: "Dispoñible proximamente",
  },
  pilgrims: {
    eyebrow: "Camiño de Santiago",
    title: "Recupera o corpo despois do Camiño",
    text: "Despois de camiñar durante días, o corpo pide descanso. En Infinity Bliss atoparás tratamentos orientados a pernas cansadas, pés sobrecargados e a recuperación tras as etapas do Camiño de Santiago.",
    cta: "Ver tratamentos para peregrinos",
  },
  about: {
    eyebrow: "O centro",
    title: "Benestar próximo, con calma",
    text: "Queremos que cada visita se sinta persoal e sen présa: un espazo limpo, sereno e próximo en Santiago de Compostela.",
    points: {
      personalized: {
        title: "Atención personalizada",
        text: "Escoitamos como chegas e adaptamos a sesión ao que o teu corpo necesita ese día.",
      },
      treatments: {
        title: "Distintos tratamentos",
        text: "Masaxes, coidados de mans e pés, e técnicas de recuperación para distintos ritmos de vida.",
      },
      atmosphere: {
        title: "Ambiente relaxante",
        text: "Un contorno tranquilo, con espazo para desconectar e volver a un ritmo máis amable.",
      },
      professionals: {
        title: "Profesionais do benestar",
        text: "Un equipo centrado no coidado do corpo, a proximidade e unha experiencia serena.",
      },
    },
    imageLabel: "O centro",
    imageAlt: "Espazo reservado para unha fotografía do interior de Infinity Bliss",
  },
  reviews: {
    eyebrow: "Opinións",
    title: "O que contan quen nos visita",
  },
  booking: {
    eyebrow: "Cita",
    title: "Reserva o teu momento",
    text: "Escolle o tratamento que mellor encaixe contigo e consulta os horarios dispoñibles.",
    secondaryText: "Reserva en liña de forma rápida e sinxela.",
    cta: "Reservar cita",
    pageEyebrow: "Reservas",
    pageTitle: "Reserva o teu momento",
    pageDescription:
      "Selecciona o teu tratamento, escolle o horario que mellor che encaixe e completa a túa reserva.",
    loadingLabel: "Cargando as reservas…",
    loadingTitle: "Preparando a túa reserva",
    loadingSubtitle:
      "Estamos cargando a dispoñibilidade para mostrarte os horarios dispoñibles.",
    slowText:
      "Está tardando un pouco máis do habitual. Podes agardar uns segundos ou abrir o sistema de reservas directamente.",
    errorText:
      "Non puidemos cargar o sistema de reservas. Podes intentalo de novo ou abrir a reserva directamente.",
    errorLinkLabel: "Abrir reservas directamente",
    retryLabel: "Reintentar",
    groupTitle: "Queres reservar para un grupo?",
    groupText:
      "Para reservas de varias persoas ou necesidades especiais, escríbenos por WhatsApp ou chámanos e axudarémosche a organizalo.",
    groupCta: "Contactar por WhatsApp",
    selectionLabel: "Estás reservando",
    changeTreatment: "Cambiar tratamento",
    metaTitle: "Reservas | Infinity Bliss",
    metaDescription:
      "Reserva masaxes e tratamentos de benestar en Infinity Bliss, Santiago de Compostela.",
  },
  contact: {
    eyebrow: "Visítanos",
    title: "Localización e contacto",
    text: "Estamos na Rúa Gómez Ulla 4, en Santiago de Compostela. Podes chamarnos ou escribirnos por WhatsApp para reservar a túa cita.",
    address: "Enderezo",
    phone: "Teléfono",
    whatsapp: "WhatsApp",
    email: "Correo electrónico",
    hours: "Horario",
    pending: "Por confirmar",
    mapLabel: "Mapa de localización",
    mapPlaceholder: "O mapa mostrarase aquí cando confirmemos o enderezo.",
  },
  footer: {
    nav: "Navegación",
    contact: "Contacto",
    legal: "Información legal",
    legalNotice: "Aviso legal",
    privacy: "Privacidade",
    cookies: "Cookies",
    bookingTerms: "Condicións de reserva",
    rights: "Todos os dereitos reservados.",
    blurb: "Centro de benestar en Santiago de Compostela",
  },
  legal: {
    notice: {
      title: "Aviso legal",
      description:
        "Aviso legal de Infinity Bliss – Centro de Benestar. Información identificativa e condicións de uso do sitio web.",
    },
    privacy: {
      title: "Política de privacidade",
      description:
        "Política de privacidade de Infinity Bliss. Información sobre o tratamento de datos persoais e reservas.",
    },
    cookies: {
      title: "Política de cookies",
      description:
        "Información sobre as cookies e tecnoloxías utilizadas na web e no sistema de reservas de Infinity Bliss.",
    },
    bookingTerms: {
      title: "Condicións de reserva e cancelación",
      description:
        "Condicións de reserva e cancelación de Infinity Bliss. Antelación, modificación, grupos e tarifas especiais.",
    },
  },
  notFound: {
    title: "Páxina non atopada",
    text: "A ligazón non existe ou cambiou. Volve ao inicio para continuar.",
    cta: "Ir ao inicio",
  },
};
